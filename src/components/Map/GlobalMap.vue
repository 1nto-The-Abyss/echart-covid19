...
export default {
  ...
  methods:{
    buttonClick1() {
      ...
      this.title = "现存确诊"
      this.$nextTick(() => {
        this.$charts.globalMap("globalChart",this.nowConfirmDate,this.title)
      })
    },
    buttonClick2() {
      this.active = false
      this.title = "累计确诊"
      this.$nextTick(() => {
        this.$charts.globalMap("globalChart",this.confirmDate,this.title)
      })
    },
    // 取出现存确诊数据并渲染
    setNowConfirm(value) {
    ...
    setConfirm(value) {
      for (let i = 0; i < value.length; i++) {
        const obj = {}
        obj.name = value[i].name
        obj.value = value[i].confirm
        this.confirmDate.push(obj)
      }
    }
  },
  mounted() {
    this.$api.getNcovGlobal().then(res => {
      const mapDate = res.data.data.WomAboard
      console.log(mapDate);
